...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Update { range_start, range_end } => {
            let search_api_url = format!("{origin}/search.json");
            let search_range_query = format!("after:{} before:{}", range_start, range_end);
            ...
            let topics_len = id_set.len();

            for (count, topic_id) in id_set.into_iter().enumerate() {
                ...
                if std::fs::metadata(topic_dir.clone()).is_ok() {
                    std::fs::remove_dir_all(topic_dir.clone())?;
                }
